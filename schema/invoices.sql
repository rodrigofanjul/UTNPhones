...
BEGIN
    ...
    DECLARE cur_calls CURSOR FOR SELECT id, id_line_origin FROM calls WHERE id_invoice IS NULL GROUP BY id_line_origin;
		...
		open cur_calls;
			...
			loop_calls : LOOP
				...
				IF(i <> phoneline_id) THEN
					SELECT SUM(call_totalprice), COUNT(*) INTO calls_totalprice, calls_quantity FROM calls WHERE id_invoice IS NULL AND id_line_origin = phoneline_id;
				...
				UPDATE calls SET id_invoice = new_invoice_id WHERE id = call_id;
